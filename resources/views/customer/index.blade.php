           ...
           <tbody>
              @forelse ($customers as $customer)
                ...
                <td class="text-center">
                  <a href="{{ route('customer.edit',$customer->id) }}" class="btn btn-sm btn-outline-danger py-0">Edit</a>
                  <a href="{{ route('customer.view',$customer->id) }}" class="btn btn-sm btn-outline-danger py-0">View</a>
                  ...
                  <form id="delete-{{$customer->slug}}" method="post" action="{{route('customer.delete',$customer->id)}}" style="display: none;">
                  @csrf
                  @method('DELETE')
                  </form>
                </td>
